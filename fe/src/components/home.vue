        ...
        <b-container>
            <b-row>
                ...
                中断
                        ...
                        <a href="javascript:void(0)" @click="clearSyncCache">
...
export default {
    ...
    data() {
        return {
            settingUrl: `http://${window.location.hostname}:8081/x/setting`,
            listenPort: 8081,
            shareUrl: '',
            token: '',
            dev: {},
            ...
            syncText: '',
            uploadOptions: {
                target: `${window.location.origin}/x/upload`,
                testChunks: false,
    ...
    methods: {
        ...
        clearSyncCache() {
            this.$http.post(this.genUrl('/setting'), {
                mode: 'clearSync',
        ...
        genUrl: function (path) {
            return `${window.location.origin}/x` + path
        },
        ...
        pingDevice: function () {
            ...
            this.$http.get(this.genUrl('/ping'), {params: {masterId: this.$route.query.masterId}}).then(resp => {
                ...
                this.shareUrl = resp.data.serInfo.shareUrl
        ...
        initWsEvent: function () {
            this.ws = new WebSocket(`ws://${window.location.hostname}:${this.listenPort}/x/ws/sync/${this.wsGroupId}`)
            this.ws.onopen = () => {
